...
set autotrace on
select customer_id, cust_last_name, gender
from oe.cust_idx_test
where customer_id=219 and gender='M';
set autotrace off

grant unlimited tablespace to oe;

create index oe.IDX_CUSTID_GENDER
on oe.cust_idx_test(customer_id,gender) tablespace idx_cust_test_tbs;

EXEC DBMS_STATS.gather_index_stats('OE', 'IDX_CUSTID_GENDER');
